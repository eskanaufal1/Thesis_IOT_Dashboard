...
from models.schemas import DeviceStatus, LogLevel, TelemetryDataCreate, SystemLogCreate
...
class TelemetryHandler(MQTTMessageHandler):
    ...
    async def handle_message(self, message: 'MQTTMessage') -> None:
        ...
        try:
            ...
            await db_service.update_device_status(message.device_id, "online")
            ...
            telemetry_data = TelemetryDataCreate(
                ...
                additional_data=message.payload.get("additional", {})
            )
            
            await db_service.store_telemetry(telemetry_data)
...
class StatusHandler(MQTTMessageHandler):
    ...
    async def handle_message(self, message: 'MQTTMessage') -> None:
        ...
        try:
            ...
            await db_service.update_device_status(message.device_id, status)
            
            # Handle relay states if provided
            relay_states = message.payload.get("relays", {})
            for relay_num, state in relay_states.items():
                # Store relay state using database service
                # This would require adding relay state creation to the database service
                pass
...
class MQTTService:
    ...
    def _setup_handlers(self) -> None:
        ...
        self._message_handlers = {
            "telemetry": TelemetryHandler(self.socketio_manager),
            "status": StatusHandler(self.socketio_manager),
            "control/response": ControlResponseHandler(self.socketio_manager),
        }
    ...
    async def _log_system_event(self, level: LogLevel, module: str, message: str, device_id: Optional[str] = None) -> None:
        """Log system events using database service"""
        try:
            ...
            await db_service.log_system_event(log_data)
        except Exception as e:
    ...
    async def publish_control_command(self, device_id: str, command: Dict[str, Any]) -> bool:
        """Legacy method - use send_control_command instead"""
        return await self.send_control_command(device_id, command)
    
    async def publish_relay_control(self, device_id: str, relay_number: int, state: bool) -> bool:
        """Legacy method - use send_relay_command instead"""
        return await self.send_relay_command(device_id, relay_number, state)
